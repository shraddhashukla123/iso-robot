...
from iso_robot.schemas.api import RiskAssessmentResponse, ScoreRisksRequest
from iso_robot.repositories.issue_control_repository import IssueControlRepository
from iso_robot.schemas.api import AssignControlsRequest, AssignControlsResponse
...
async def assign_issue_controls(
    issue_id: str,
    request: AssignControlsRequest,
    db: Annotated[aiosqlite.Connection, Depends(get_db)],
) -> AssignControlsResponse:
    repo = IssueControlRepository(db)
    await repo.clear(issue_id)
    n = await repo.assign(issue_id, request.control_ids)
    return AssignControlsResponse(issue_id=issue_id, assigned=n)